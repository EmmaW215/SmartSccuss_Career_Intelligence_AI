"""
GPU Server Client (Phase 2 - Optional)
...
class GPUClient:
    """
    Client for GPU server communication (optional)
    ...
    async def check_health(self, force: bool = False) -> Dict[str, Any]:
        ...
        if not force and self._status_cache:
            if current_time - self._status_cache.last_check < self.health_cache_ttl:
                return {
        ...
        try:
            ...
            response = await client.get(f"{self.gpu_url}/health", timeout=5.0)
            ...
            if response.status_code == 200:
                ...
                self._status_cache = GPUStatus(
                    available=True,
                    latency_ms=latency,
                    services=data.get("services", {"stt": True, "tts": True, "rag": True}),
                    last_check=current_time
                )
                return {
                    "available": True,
        ...
        return {
            "available": False,
            "error": self._status_cache.error if self._status_cache else "Unknown error",
            "fallback": "edge_tts"
        }
    ...
    async def transcribe(
        ...
        Args:
            audio_data: Audio bytes (webm, wav, etc.)
            language: Language code
            
        Returns:
            Tuple of (transcript, provider_used)
        """
        status = await self.check_health()
        
        if status.get("available") and status.get("services", {}).get("stt", True):
            try:
                transcript = await self._gpu_transcribe(audio_data, language)
                return transcript, VoiceProvider.GPU
            except Exception as e:
                print(f"GPU STT failed: {e}")
        
        # No fallback for STT - voice input requires GPU
        raise Exception(
            "Speech-to-text unavailable. GPU server offline. "
            "Please use text input mode or start GPU server."
        )
    ...
    async def synthesize(
        ...
        if status.get("available") and status.get("services", {}).get("tts", True):
            try:
        ...
        try:
            ...
            raise Exception("Text-to-speech unavailable. All providers failed.")
    ...
    async def build_custom_rag(
        ...
        try:
            ...
            for i, file_info in enumerate(files):
                ...
                files_data.append((
                    "files",
                    (file_info.get("filename", f"file_{i}"), content, 
            ...
            if response.status_code != 200:
                raise Exception(f"RAG build failed: {response.text[:200]}")
            
            return response.json()
        except httpx.TimeoutException:
            raise Exception("RAG building timed out. Try smaller documents.")
    
    async def _gpu_transcribe(self, audio_data: bytes, language: str) -> str:
    ...
    async def _gpu_synthesize(
        ...
        response = await client.post(
            ...
            json={"text": text, "voice": voice, "emotion": emotion, "model": "xtts-v2"},
            timeout=30.0
        )
