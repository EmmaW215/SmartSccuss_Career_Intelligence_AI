...
import { Beaker, ArrowRight, CheckCircle2, Home, Search, Filter, ChevronDown, Clock, FileText, Target, CheckSquare, Award, Lock } from 'lucide-react';
import SimpleVisitorCounter from '../components/SimpleVisitorCounter';
import { LAB_CHALLENGES } from '../constants/labChallenges';
import { LabChallenge } from '../types';
import { useAuth } from '../contexts/AuthContext';

interface LabSkillsLabPageProps {
  onBackToHome: () => void;
  onSelectChallenge: (challenge: LabChallenge) => void;
...
export const LabSkillsLabPage: React.FC<LabSkillsLabPageProps> = ({ onBackToHome, onSelectChallenge, completedChallenges }) => {
  const [selectedTask, setSelectedTask] = useState<string | null>(null);
  const { isAuthenticated, isPro, triggerLogin, triggerUpgrade } = useAuth();

  const handleSelectChallenge = (challenge: LabChallenge) => {
    if (!isAuthenticated) {
      triggerLogin();
      return;
    }
    ...
    // Only Pro users can proceed
    onSelectChallenge(challenge);
  };

  const handleSelectTask = (challengeId: string) => {
    // Check authentication before allowing to view details
    if (!isAuthenticated) {
      triggerLogin();
      return;
    }
    ...
    // Only Pro users can view challenge details
    setSelectedTask(challengeId === selectedTask ? null : challengeId);
  };
              ...
              >
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      onClick={() => handleSelectTask(challenge.id)}
                ...
                {selectedTask === challenge.id && (
                  <div className="px-6 pb-6 pt-2 border-t border-gray-50 bg-purple-50/30 animate-in slide-in-from-top-4 duration-300">
                    ...
                    <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mt-4">
                      ...
                      {/* Show lock icon and message for non-Pro users */}
                      {!isPro && (
                        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-2">
                          <Lock size={16} className="text-amber-600" />
                          <span className="text-sm text-amber-800">Pro subscription required to access the workspace.</span>
                        </div>
                      )}
                      ...
                      <div className="mt-4 flex justify-end gap-3">
                        ...
                        >
                          {isCompleted ? 'Re-enter Lab' : 'Enter Lab'} 
                          {!isPro && <Lock size={14} />}
                          {isPro && <ArrowRight size={14} />}
                        </button>
                      </div>
